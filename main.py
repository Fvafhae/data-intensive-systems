...
def main():
    ...
    string_sim = s.StringSimilarity(jaro_or_edit="jaro", edit_th=5, jaro_th=0.5)
    string_sim.run()
    ...
    minhasher = MinHashLSHProcessor(spark_session=string_sim.spark, sparse_vector_df=shing.sparse_vectors_df, jaccard_th=0.1)
    minhasher.run()
