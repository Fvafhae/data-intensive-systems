...
def tester(file_path):
    ...
    for i in range(0, test_case_count):
        ...
        for jaro_loop in tqdm(range(6, 11)):
            jaro_th = jaro_loop * 0.1
            # investigate jaccard dist 0.1 - 0.6
            for jaccard_loop in tqdm(range(1, 4), leave=False):
                jaccard_th = jaccard_loop * 0.1
                st_solution = time.time()

                for sig_size in tqdm([5, 10, 15, 20], leave=False):
                    spark_session = spark_config._create_spark_session()

                    # string similarity is used
                    string_sim = s.StringSimilarity(spark=spark_session, jaro_th=jaro_th)
                    string_sim.run()
                    # string_sim.collapsed_data.show(truncate=False)

                    input_length = string_sim.input_length

                    shing = Shingler(spark_session = spark_session, df = string_sim.collapsed_data)
                    shing.run()

                    minhasher = MinHashLSHProcessor(spark_session=spark_session, sparse_vector_df=shing.sparse_vectors_df, jaccard_th=jaccard_th, signature_size=sig_size)
                    minhasher.run()
                    minhasher.final_similarity_groups.toPandas().to_csv('final_similarity_groups.csv')

                    solution_time = time.time() - st_solution

                    acc_calculator = CalculateAccuracy(spark_session=spark_session)
